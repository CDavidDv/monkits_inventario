            ...
            <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
                ...
                <div v-if="modalType !== 'assign'" class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input v-model="formData.codigo_barras" type="text"
                ...
                <div v-if="modalType !== 'assign'">
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Imágenes
                        <span class="text-xs text-gray-400 font-normal ml-1">(nombres de archivo separados por comas)</span>
                    </label>
                    <input v-model="formData.imagenes" type="text"
                        placeholder="Ej: tuerca 3mm.webp, tuerca-lateral.jpg"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                    <!-- Vista previa de nombres -->
                    <div v-if="imageList.length > 0" class="mt-2 flex flex-wrap gap-1">
                        <span v-for="img in imageList" :key="img"
                            class="inline-flex items-center px-2 py-0.5 bg-gray-100 border border-gray-300 rounded text-xs text-gray-600">
...
// Lista de nombres de imagen para la vista previa
const imageList = computed(() => {
    if (!formData.imagenes) return []
    return formData.imagenes.split(',').map(s => s.trim()).filter(s => s.length > 0)
})
...
watch([() => props.item, () => props.modalType], ([newItem, newModalType]) => {
    ...
    if (newItem && newModalType === 'edit') {
        Object.assign(formData, {
            ...
            codigo_barras: newItem.codigo_barras || '',
            imagenes: newItem.imagenes || '',
